...
export async function updateCategory(
  ...
  if (!category)
    throw new AppError(
      "Category not found",
      404,
      "Can not find category with this Id"
    );
  if (
    category.getDataValue("type") === "global" &&
    user?.getDataValue("role") !== "admin"
  ) {
    throw new AppError(
      "Only admins can modify global categories",
      403,
      "FORBIDDEN"
    );
  }
  if (
    category.getDataValue("type") === "custom" &&
    user?.getDataValue("id") !== category.getDataValue("userId")
  ) {
    throw new AppError(
      "Only the user can delete his own custom categories",
      403,
      "FORBIDDEN"
    );
  }
...
export async function deleteCategory(categoryId: string, userId: string) {
  ...
  if (
    ...
    throw new AppError(
      "Only user can delete this associated custom categories",
      403,
      "FORBIDDEN"
    );
  }
...
export async function createCategory(userId: string, data: CreateCategoryDto) {
  const user = await userRepository.findById(userId);
  if (data.type === "global" && user?.getDataValue("role") !== "admin") {
    throw new AppError(
      "Only admins can carete global categories",
      403,
      "FORBIDDEN"
    );
  }

...
export async function login(email, password) {
    ...
    console.log(user);
    if (!user)
        throw new AppError("Invalid email or password", 401, "can not find the user with the given credentials");
...
export async function refreshToken(refreshToken) {
    ...
    if (!user)
        throw new AppError("User not found", 404, "No user found with the given ID");
